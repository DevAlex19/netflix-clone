...
function Slider() {
  ...
  function handleNext() {
    ...
    const newData = [
      ...cards.data.slice(3, cards.data.length - 1),
  ...
  return (
    <Box sx={{ overflow: "hidden", height: "200px", position: "relative" }}>
      <Box
        sx={{
          display: "flex",
          columnGap: "7px",
          transform: `translateX(-${cards.index * 50}%)`,
          transition: cards.transition ? "transform 0.7s ease-in" : "none",
        }}
        onTransitionEnd={handleTransitionEnd}
      >
        {cards.data.map((item: string, index: number) => {
          return <img style={{ width: "16%" }} key={index} src={item} />;
        })}
      </Box>
      <Box
        sx={{
        ...
        onClick={() => {
          ...
          // if (Math.abs(newIndex) + 6 >= cards.data.length - 6) {
          //   const newCards = [
          //     ...cards.data.slice(cards.data.length - 6, cards.data.length - 1),
          //     ...cards.data.slice(0, cards.data.length - 5),
          //   ];
          //   setCards({ ...cards, data: [...newCards] });
          //   return;
          // }
        }}
      >{`<`}</Box>
